...
function runStaticChecks(source: string): StaticResult {
  ...
  // Gate 3 parseability check (warn-only on absence; error on malformed)
  const gate3Header = /\/\/\s*Endings\s*\(locked\s*at\s*Gate\s*3\)/i;
  if (!gate3Header.test(source)) {
    diagnostics.push({ severity: "warn", message: `No "// Endings (locked at Gate 3):" comment block found — distribution will be informational only` });
  }
...
function parseGate3(source: string): Map<string, number> | null {
  const headerIdx = source.search(/\/\/\s*Endings\s*\(locked\s*at\s*Gate\s*3\)/i);
...
function main() {
  ...
  const source = readFileSync(flags.inkFile, "utf8");

  if (!flags.quiet) process.stderr.write(`[verify-ink] ${flags.inkFile}\n`);
